...
#include "caffe/util/ssd_io.hpp"
#include "caffe/util/ssd_transformer.hpp"

namespace caffe {

template<typename Dtype>
SSDTransformer<Dtype>::SSDTransformer(const TransformationParameter& param,
    Phase phase)
...
template<typename Dtype>
void SSDTransformer<Dtype>::Transform(const Datum& datum,
...
template<typename Dtype>
void SSDTransformer<Dtype>::Transform(const vector<Datum> & datum_vector,
                                       Blob<Dtype>* transformed_blob) {
...
template<typename Dtype>
void SSDTransformer<Dtype>::Transform(
...
template<typename Dtype>
void SSDTransformer<Dtype>::TransformAnnotation(
...
template<typename Dtype>
void SSDTransformer<Dtype>::CropImage(const Datum& datum,
...
template<typename Dtype>
void SSDTransformer<Dtype>::CropImage(const AnnotatedDatum& anno_datum,
                                       const NormalizedBBox& bbox,
...
template<typename Dtype>
void SSDTransformer<Dtype>::ExpandImage(const Datum& datum,
...
template<typename Dtype>
void SSDTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
...
template<typename Dtype>
void SSDTransformer<Dtype>::DistortImage(const Datum& datum,
                                          Datum* distort_datum) {
...
#ifdef USE_OPENCV
template<typename Dtype>
void SSDTransformer<Dtype>::Transform(const vector<cv::Mat> & mat_vector,
                                       Blob<Dtype>* transformed_blob) {
...
template<typename Dtype>
void SSDTransformer<Dtype>::Transform(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob,
                                       NormalizedBBox* crop_bbox,
                                       bool* do_mirror) {
  // Check dimensions.
...
template<typename Dtype>
void SSDTransformer<Dtype>::TransformInv(const Dtype* data, cv::Mat* cv_img,
...
template<typename Dtype>
void SSDTransformer<Dtype>::TransformInv(const Blob<Dtype>* blob,
...
template <typename Dtype>
void SSDTransformer<Dtype>::CropImage(const cv::Mat& img,
                                       const NormalizedBBox& bbox,
                                       cv::Mat* crop_img) {
...
template <typename Dtype>
void SSDTransformer<Dtype>::ExpandImage(const cv::Mat& img,
                                         const float expand_ratio,
                                         NormalizedBBox* expand_bbox,
                                         cv::Mat* expand_img) {
  const int img_height = img.rows;
...
template<typename Dtype>
void SSDTransformer<Dtype>::Transform(Blob<Dtype>* input_blob,
                                       Blob<Dtype>* transformed_blob) {
  const int crop_size = param_.crop_size();
...
template<typename Dtype>
vector<int> SSDTransformer<Dtype>::InferBlobShape(const Datum& datum) {
  if (datum.encoded()) {
#ifdef USE_OPENCV
    CHECK(!(param_.force_color() && param_.force_gray()))
        << "cannot set both force_color and force_gray";
...
template<typename Dtype>
vector<int> SSDTransformer<Dtype>::InferBlobShape(
...
#ifdef USE_OPENCV
template<typename Dtype>
vector<int> SSDTransformer<Dtype>::InferBlobShape(const cv::Mat& cv_img) {
  const int crop_size = param_.crop_size();
  int crop_h = param_.crop_h();
  int crop_w = param_.crop_w();
  if (crop_size) {
...
template <typename Dtype>
void SSDTransformer<Dtype>::InitRand() {
...
template <typename Dtype>
int SSDTransformer<Dtype>::Rand(int n) {
  CHECK(rng_);
  CHECK_GT(n, 0);
  caffe::rng_t* rng =
...
INSTANTIATE_CLASS(SSDTransformer);
