...
class MultiBoxLossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  void Fill(bool share_location) {
    ...
    // Fake bottom gt to avoid referencing destroyed memory
    // Refer to https://github.com/weiliu89/caffe/issues/427#issuecomment-287290498
    Blob<Dtype>* fake_bottom_gt = new Blob<Dtype>();
    ...
    fake_top_vec.push_back(fake_bottom_gt);
    ...
    caffe_copy(fake_bottom_gt->count(), fake_bottom_gt->cpu_data(),
        blob_bottom_gt_->mutable_cpu_data());
#else
